...
namespace TVRename
{
    public class SeriesInfo : ISelectableShowPart
    {
        ...
        public SeriesInfo(ShowLibrary d,string name, int id)
        {
            mDoc = d;
            SetToDefaults();
            Name = name;
        ...
        public SeriesInfo(ShowLibrary d, string name, int id, string langCode)
        {
            mDoc = d;
            SetToDefaults();
        ...
        public SeriesInfo(ShowLibrary d, [NotNull] XElement seriesXml)
        {
            mDoc = d;
            SetToDefaults();
            LoadXml(seriesXml);
        ...
        public SeriesInfo(ShowLibrary d, [NotNull] JObject json,int langId)
        {
            mDoc = d;
            SetToDefaults();
            LanguageId = langId;
        ...
        public SeriesInfo(ShowLibrary d, [NotNull] JObject json, JObject jsonInDefaultLang, int langId)
        {
            mDoc = d;
            SetToDefaults();
            LanguageId = langId;
        ...
        [NotNull]
        public IEnumerable<string> ActorNames => GetActors().Select(x => x.ActorName);

        private ShowLibrary mDoc;

        public ShowItem Show => mDoc.ShowItem(TvdbCode);
        ...
        private void LoadJson([NotNull] JObject r)
        {
            ...
            {
                Name = System.Net.WebUtility.HtmlDecode((string)r["seriesName"]);
            }
        ...
        private void LoadJson([NotNull] JObject bestLanguageR, JObject backupLanguageR)
        {
            ...
            if ((string.IsNullOrWhiteSpace(Name) && ((string)backupLanguageR["seriesName"] != null)) ){
                Name = System.Net.WebUtility.HtmlDecode((string)backupLanguageR["seriesName"]);
            }

            if ((string.IsNullOrWhiteSpace(Overview) && ((string)backupLanguageR["overview"] != null)) ){
                Overview = System.Net.WebUtility.HtmlDecode((string)backupLanguageR["overview"]);
            }
