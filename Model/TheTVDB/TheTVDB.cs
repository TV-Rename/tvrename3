...
namespace TVRename
{
    ...
    public class TheTVDB : iTVSource
    {
        ...
        public void Setup(ShowLibrary d, [CanBeNull] FileInfo loadFrom, [NotNull] FileInfo cache, CommandLineArgs cla)
        {
            args = cla;
            ...
            LoadOk = (loadFrom is null) || LoadCache(d,loadFrom);
        ...
        [CanBeNull]
        public SeriesInfo GetSeriesAndDownload(ShowLibrary d, int id) => HasSeries(id) ? series[id] : DownloadSeriesNow(d,id,false,false,false,TVSettings.Instance.PreferredLanguageCode);
        ...
        private bool LoadCache(ShowLibrary d, [NotNull] FileInfo loadFrom)
        {
        ...
        public SeriesInfo GetSeries(ShowLibrary d, string showName, bool showErrorMsgBox)
        {
            Search(d,showName,showErrorMsgBox);
        ...
        public void ForgetShow(ShowLibrary d, int id, bool makePlaceholder,bool useCustomLanguage, string customLanguageCode)
        {
            lock (SERIES_LOCK)
            {
                if (series.ContainsKey(id))
                {
                    ...
                    if (makePlaceholder)
                    {
                        ...
                        {
                            AddPlaceholderSeries(d,id, name, customLanguageCode);
                        }
                        else
                        {
                            AddPlaceholderSeries(d,id, name);
                        }
        ...
        public bool GetUpdates(bool showErrorMsgBox)
        {
            ...
            while (moreUpdates)
            {
                ...
                catch (InvalidCastException ex)
                {
                    ...
                    {
                        MessageBox.Show(msg, "Error obtaining updates from TVDB", MessageBoxButton.OK,
                ...
                if (numberofCallsMade > MAX_NUMBER_OF_CALLS)
                {
                    ...
                    if ((!args.Unattended) && (!args.Hide) && Environment.UserInteractive)
                    {
                        MessageBox.Show(errorMessage, "Long Running Update", MessageBoxButton.OK,
                            MessageBoxImage.Warning);
                    }
        ...
        private bool ProcessXml(ShowLibrary d, [NotNull] XElement x)
        {
            ...
            //</Data>
                ...
                try
                {
                    ...
                    foreach (SeriesInfo si in x.Descendants("Series").Select(seriesXml => new SeriesInfo(d,seriesXml)))
                    {
        ...
        private SeriesInfo DownloadSeriesNow([NotNull] SeriesSpecifier deets, bool episodesToo, bool bannersToo) =>
            DownloadSeriesNow(deets.doc,deets.SeriesId, episodesToo, bannersToo, deets.UseCustomLanguage,
                deets.CustomLanguageCode);

        [CanBeNull]
        private SeriesInfo DownloadSeriesNow(ShowLibrary d, int code, bool episodesToo, bool bannersToo, bool useCustomLangCode, string langCode)
        {
            if (code == 0)
            {
                Say("");
            ...
            try
            {
                si = DownloadSeriesInfo(d,code, requestedLanguageCode);
            }
        ...
        [NotNull]
        private SeriesInfo DownloadSeriesInfo(ShowLibrary d, int code, [NotNull] string requestedLanguageCode)
        {
            ...
            SeriesInfo si = GenerateSeriesInfo(d,jsonResponse, jsonDefaultLangResponse, isNotDefaultLanguage, requestedLanguageCode);
        ...
        [NotNull]
        private SeriesInfo GenerateSeriesInfo(ShowLibrary d, [NotNull] JObject jsonResponse, JObject jsonDefaultLangResponse, bool isNotDefaultLanguage,
            [NotNull] string requestedLanguageCode)
        {
            ...
            if (isNotDefaultLanguage)
            {
                ...
                si = new SeriesInfo(d,seriesData, seriesDataDefaultLang, requestedLangId);
            }
            else
            {
                si = new SeriesInfo(d,seriesData, GetLanguageId());
            }
        ...
        private void AddPlaceholderSeries(ShowLibrary d, int code, [CanBeNull] string name)
        {
            series[code] = new SeriesInfo(d,name ?? "", code) {Dirty = true};
        }

        private void AddPlaceholderSeries(ShowLibrary d,int code, [CanBeNull] string name,string customLanguageCode)
        {
            series[code] = new SeriesInfo(d,name ?? "", code, customLanguageCode) { Dirty = true };
        }
        ...
        public void Search(ShowLibrary d, string text, bool showErrorMsgBox)
        {
            if (!Connected && !Connect(showErrorMsgBox))
            {
            ...
            try
            {
                if (isNumber)
                {
                    ...
                    {
                        DownloadSeriesNow(d,textAsInt, false, false, false,
                            TVSettings.Instance.PreferredLanguageCode);
                    }
                }
            }
            ...
            {
                ProcessSearchResult(d,uri, jsonSearchResponse,GetLanguageId());
            }

            if (jsonSearchDefaultLangResponse != null)
                //we also want to search for search terms that match in default language
            {
                ProcessSearchResult(d,uri, jsonSearchDefaultLangResponse,GetDefaultLanguageId());
            }
        }

        private void ProcessSearchResult(ShowLibrary d, string uri, [NotNull] JObject jsonResponse, int languageId)
        {
            try
            {
                foreach (SeriesInfo si in jsonResponse["data"]
                    .Cast<JObject>()
                    .Select(seriesResponse => new SeriesInfo(d,seriesResponse, languageId)))
                {
                    lock(SERIES_LOCK)
                    {
                        if (series.ContainsKey(si.TvdbCode))
