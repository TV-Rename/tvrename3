...
namespace TVRename
{
    internal static class ShowHtmlHelper
    {
        ...
        {
            Color col = SystemColors.ButtonFace;
            ...
            string html = "<html><head><meta http-equiv=\"X-UA-Compatible\" content=\"IE=11\" ><meta charset=\"UTF-8\"><STYLE type=\"text/css\">" + css + "</style>";
        ...
        private static void AppendShow(this StringBuilder sb,[CanBeNull] ShowItem si, Color backgroundColour, bool includeDirectoryLinks)
        {
            ...
            string explorerButton = includeDirectoryLinks
                ? CreateButton($"{MainWindow.EXPLORE_PROXY}{urlFilename}", "<i class=\"far fa-folder-open\"></i>", "Open Containing Folder")
                : string.Empty;
        ...
        internal static string SeasonName(Season s)
        {
            return SeasonName(s.TheSeries.Show, s.SeasonNumber);
        }
        ...
        private static void AppendEpisode([NotNull] this StringBuilder sb, [NotNull] ProcessedEpisode ep, [CanBeNull] IReadOnlyCollection<FileInfo> fl,Color backgroundColour)
        {
            ...
            if (fl != null)
            {
                foreach (FileInfo fi in fl)
                {
                    ...
                    viewButton += CreateButton($"{MainWindow.WATCH_PROXY}{urlFilename}", "<i class=\"far fa-eye\"></i>","Watch Now");
                    explorerButton += CreateButton($"{MainWindow.EXPLORE_PROXY}{urlFilename}", "<i class=\"far fa-folder-open\"></i>","Open Containing Folder");
                }
            }
        ...
        public static string GetSeasonHtmlOverviewOffline([NotNull] this ShowItem si, [NotNull] Season s)
        {
            ...
            foreach (ProcessedEpisode ei in eis)
            {
                ...
                {
                    body += "<b>" + WebUtility.HtmlEncode(CustomEpisodeName.NameForNoExt(ei, CustomEpisodeName.OldNStyle(6))) +
                            "</b>";
                }
                ...
                if (fl.Count>0)
                {
                    foreach (FileInfo fi in fl)
                    {
                        string urlFilename = WebUtility.UrlEncode(fi.FullName);
